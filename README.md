...
- KIS는 초당 호출 횟수 제한이 빡빡함(모의투자 계좌에서 `EGW00201` 레이트리밋을 실제로
  겪음). 그래서 API 호출 사이에 1.1초 간격을 둠. 거래량순위 응답에 종목별 현재가
  (`stck_prpr`)가 이미 포함돼 있어서 후보마다 따로 현재가 조회를 하지 않고 그 값을
  그대로 씀 — 스캔당 호출은 순위조회 1번 + 후보당 일봉조회 1번뿐이라
  `watchlist_size × 1.1초` 정도로 절반 가까이 줄었음. mock 모드는 실제 API가
  아니라서 이 간격이 없음.
